...
import com.flius.guiddy.databinding.ActivityFoMainBinding
...
class FoMainActivity : AppCompatActivity() {
    lateinit var binding: ActivityFoMainBinding
    lateinit var adapter: FoPostListAdapter
    private lateinit var mAuth: FirebaseAuth
    private lateinit var mDbRef: DatabaseReference
    private lateinit var postList: ArrayList<Pair<Profile, Intro>>
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityFoMainBinding.inflate(layoutInflater)
        setContentView(binding.root)

        mAuth = Firebase.auth
        mDbRef = Firebase.database.reference
        postList = ArrayList()
        adapter = FoPostListAdapter(this, postList)
    ...
    private fun showLogoutConfirmationDialog() {
        AlertDialog.Builder(this).apply {
            ...
            setPositiveButton("예") { _, _ ->
                // 로그인 화면으로 이동
                mAuth.signOut()
                val intent = Intent(this@FoMainActivity, LoginActivity::class.java)
